...
const stateStyles = {
  LOW: {
    frame: 'border-institute-500/25 bg-institute-500/10',
    track: 'bg-institute-500/18',
    fill: 'bg-institute-500',
    label: 'text-institute-500'
  },
  ELEVATED: {
    frame: 'border-caution-500/25 bg-caution-500/10',
    track: 'bg-caution-500/18',
    fill: 'bg-caution-500',
    label: 'text-caution-500'
  },
  CRITICAL: {
    frame: 'border-critical-500/28 bg-critical-500/12',
    track: 'bg-critical-500/18',
...
export default function RiskMeter({ level = 'LOW', score = 0, compact = false }) {
  ...
  return (
    <div className={`rounded-[1.4rem] border ${styles.frame} px-4 py-4 shadow-[0_0_0_1px_rgba(255,255,255,0.03)] transition duration-300 ${compact ? 'px-3 py-3' : ''}`}>
      ...
      <div className={`mt-3 grid grid-cols-3 gap-2 ${compact ? 'h-8' : 'h-11'}`}>
        {riskOrder.map((itemLevel, index) => {
          ...
          return (
            <div key={itemLevel} className={`relative overflow-hidden rounded-[0.95rem] border ${active ? `${styles.track} border-transparent` : 'border-graphite-100/60 bg-graphite-50/50'}`}>
              <div className={`absolute inset-y-0 left-0 rounded-[0.95rem] ${active ? styles.fill : ''}`} style={{ width: active ? '100%' : '0%' }} />
              <div className={`relative z-10 flex h-full min-w-0 items-center justify-between gap-1 px-2 sm:px-2.5 font-mono text-[10px] uppercase tracking-[0.08em] ${active ? 'text-graphite-50' : 'text-graphite-400'}`}>
                <span className="truncate font-medium">{itemLevel}</span>
                <span className="shrink-0 text-[9px] opacity-85">{isCurrent ? '●' : index + 1}</span>
              </div>
            </div>
          )
        })}
